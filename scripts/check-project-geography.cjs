...
const roende=geometries.get('roende-etape-5');assert.equal(roende.metadata.precision,'schematic');assert.deepEqual(roende.metadata.projectIds,['syddjurs-spildevand:1']);assert.equal(roende.features.length,1);assert.equal(roende.features[0].geometry.type,'Polygon');assert.match(roende.metadata.note,/ikke bruges som matrikel/i);
...
context.pr=projects.find(p=>p.id==='syddjurs-spildevand:1');assert.ok(evaluate('projectGeometry(pr)'));assert.equal(evaluate('projectGeometryKey(pr)'),'roende-etape-5');assert.equal(evaluate('projectGeometryRole(projectGeometry(pr))'),'project-area');
...
for(const id of ['fors:0','syddjurs-spildevand:1']){context.pr=projects.find(p=>p.id===id);context.pr={...context.pr,name:'Reordered/new project'};assert.equal(evaluate('projectReview(pr)'),null);assert.equal(evaluate('projectGeometry(pr)'),null);}
...
console.log('PROJECT_GEOGRAPHY_OK',reviews.length,'classifications; 4 registered geometry datasets; source, schematic route and two schematic area datasets validated; unsafe fallback and stale identities rejected');