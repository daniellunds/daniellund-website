...
function canonicalProfileBrandId(id){
  ...
  const current=currentOperatorForBrand(id);
...
function operatorBrandView(id){
  const canonical=canonicalProfileBrandId(id);
  const base=state.brandById.get(canonical)||state.brandById.get(id);
  if(!base)return null;
  ...
  const current=typeof currentOperatorForBrand==="function"?currentOperatorForBrand(canonical):null;
  return {
    ...base,
    name:current?.displayName||base.name,
    organizationType:current?.organizationType||"utility",
    canonicalOrganizationId:current?.canonicalOrganizationId||`org:${canonical}`,
    municipalities,
...
function renderProfileOverview(b,p){
  ...
  return `${p.summary?`<p class="profile-summary">${profileEscape(p.summary)}</p>`:""}
    <div class="fact-grid profile-facts">
      ...
      <div class="fact"><span>Renseanlægsrelationer</span><strong>Afventer særskilt verificering</strong></div>
