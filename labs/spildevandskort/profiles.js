...
function operatorBrandView(id){
  ...
  return {...base,municipalities,_operatorMemberIds:memberIds};
}
...
function renderProfileOverview(b,p){
  ...
  return `${p.summary?`<p class="profile-summary">${profileEscape(p.summary)}</p>`:""}
    <div class="fact-grid profile-facts">
      ...
      <div class="fact"><span>Aktive renseanlæg</span><strong>${(b._operatorMemberIds||[b.id]).reduce((sum,id)=>sum+activePlantCountForBrand(id),0)}</strong></div>
