...
function projectGeometryRole(geometry){
  if(geometry?.metadata?.geometryRole)return geometry.metadata.geometryRole;
  const types=new Set((geometry?.features||[]).map(f=>f.geometry?.type));
  return [...types].some(t=>t==="Polygon"||t==="MultiPolygon")?"project-area":"route";
}
function projectGeometryListLabel(geometry){
  const schematic=projectGeometryPrecision(geometry)==="schematic";
  const area=projectGeometryRole(geometry)==="project-area";
  if(schematic&&area)return "Skematisk projektområde";
  if(schematic)return "Skematisk tracé";
  return area?"Dokumenteret projektområde":"Dokumenteret tracé";
}
...
function projectAreaStyle(geometry){
  const schematic=projectGeometryPrecision(geometry)==="schematic";
  return schematic?{color:"#d47c22",weight:2,opacity:.95,dashArray:"6 5",fillColor:"#d47c22",fillOpacity:.14,className:"project-area project-area-schematic"}:{color:"#08798a",weight:2,opacity:.95,fillColor:"#08798a",fillOpacity:.14,className:"project-area project-area-source"};
}
function projectFeatureStyle(feature,geometry){
  const t=feature.geometry?.type;
  if(t==="Polygon"||t==="MultiPolygon")return projectAreaStyle(geometry);
  if(t==="Point")return projectPointStyle(geometry);
  return projectLineStyle(feature,geometry);
}
...
function projectRowElement(pr){
  ...
  if(geometry){row.dataset.geometryPrecision=projectGeometryPrecision(geometry);row.dataset.geometryRole=projectGeometryRole(geometry);}
...
function openProject(pr){
  ...
  const geometryPrecision=projectGeometryPrecision(geometry),geometryRole=projectGeometryRole(geometry);
  const precision=geometry?(geometryPrecision==="schematic"?(geometryRole==="project-area"?"Skematisk, kildebaseret projektområde":"Skematisk, kildebaseret tracé"):"Offentliggjort projektgeometri"):projectPrecisionLabel(loc?.precision);
